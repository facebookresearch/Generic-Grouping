    ...
    for demokw in test_settings:
        try:
            ...
            bboxes_list = self.refine_bboxes(
                rois, labels, bbox_preds, pos_is_gts, img_metas
            )
...
def test_mask_head_loss():
    ...
    self = FCNMaskHead(
        num_convs=1, roi_feat_size=6, in_channels=8, conv_out_channels=8, num_classes=8
    )
    ...
    mask_iou_targets = mask_iou_head.get_targets(
        sampling_results, gt_masks, pos_mask_pred, mask_targets, train_cfg
    )
...
def _dummy_bbox_sampling(proposal_list, gt_bboxes, gt_labels):
    ...
    for i in range(num_imgs):
        ...
        sampling_result = bbox_sampler.sample(
            assign_result, proposal_list[i], gt_bboxes[i], gt_labels[i], feats=feat
        )
...
def test_yolact_head_loss():
    ...
    feat = [
        torch.rand(1, 256, feat_size, feat_size) for feat_size in [69, 35, 18, 9, 5]
    ]
    ...
    empty_mask_loss = mask_head.loss(
        mask_pred, gt_masks, gt_bboxes, img_metas, sampling_results
    )
