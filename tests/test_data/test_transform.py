...
def test_min_iou_random_crop():
    ...
    ious_ignore = bbox_overlaps(
        patch.reshape(-1, 4), results["gt_bboxes_ignore"]
    ).reshape(-1)
...
def test_cutout():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="CutOut", n_holes=1, cutout_shape=(2, 2), cutout_ratio=(0.4, 0.4)
        )
        build_from_cfg(transform, PIPELINES)
    ...
    transform = dict(
        type="CutOut", n_holes=1, cutout_ratio=(0.8, 0.8), fill_in=(255, 255, 255)
    )
    cutout_module = build_from_cfg(transform, PIPELINES)
    cutout_result = cutout_module(copy.deepcopy(results))
    assert cutout_result["img"].sum() > img.sum()
