...
def _check_fields(results, results_rotated, keys):
    for key in keys:
        if isinstance(results[key], (BitmapMasks, PolygonMasks)):
            assert np.equal(
                results[key].to_ndarray(), results_rotated[key].to_ndarray()
            ).all()
        else:
...
def check_rotate(results, results_rotated):
    ...
    if "gt_labels" in results:
        assert np.equal(results["gt_labels"], results_rotated["gt_labels"]).all()
...
def test_rotate():
    ...
    transform = dict(
        type="Rotate", level=10, prob=0.0, img_fill_val=img_fill_val, scale=0.6
    )
    rotate_module = build_from_cfg(transform, PIPELINES)
    results_wo_rotate = rotate_module(copy.deepcopy(results))
    check_rotate(results, results_wo_rotate)
    ...
    img_r = np.array(
        [[img_fill_val, 6, 2, img_fill_val], [img_fill_val, 7, 3, img_fill_val]]
    ).astype(np.uint8)
