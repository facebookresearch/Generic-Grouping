...
def test_dataset_evaluation():
    ...
    coco_cfg = dict(
        type="CocoDataset", ann_file=fake_json_file, classes=("car",), pipeline=[]
    )
    ...
    # build concat dataset through two config dict
    custom_cfg = dict(
        type="CustomDataset", ann_file=fake_pkl_file, classes=("car",), pipeline=[]
    )
    ...
    concat_cfg = dict(
        type="ConcatDataset", datasets=[custom_cfg, custom_cfg], separate_eval=False
    )
    concat_dataset = build_dataset(concat_cfg)
    eval_results = concat_dataset.evaluate(fake_concat_results, metric="mAP")
    assert eval_results["mAP"] == 1
    assert len(concat_dataset.datasets[0].data_infos) == len(
        concat_dataset.datasets[1].data_infos
    )
