...
def _pad(h, w, c, pad_val, axis=-1, dtype=np.float32):
    ...
    pad_data = np.stack(
        [np.ones((h, w)) * pad_val[i] for i in range(c)], axis=axis
    ).astype(dtype)
    return pad_data
...
def test_translate():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="Translate", level=1, img_fill_val=128, max_translate_offset=(250.0,)
        )
        build_from_cfg(transform, PIPELINES)
    ...
    def _check_bbox_mask(results, results_translated, offset, direction, min_size=0.0):
        ...
        for key in results_translated.get("bbox_fields", []):
            ...
            if direction == "horizontal":
                masks_pad = _pad(
                    h, abs(offset), masks.shape[0], 0, axis=0, dtype=masks.dtype
                )
                if offset <= 0:
                    # left shift
                ...
                masks_pad = _pad(
                    abs(offset), w, masks.shape[0], 0, axis=0, dtype=masks.dtype
                )
                if offset <= 0:
                    # top shift
                    gt_masks = np.concatenate(
    ...
    for key in results.get("mask_fields", []):
        ...
        assert np.equal(
            results_translated[key].to_ndarray(), translated_gt.to_ndarray()
        ).all()
